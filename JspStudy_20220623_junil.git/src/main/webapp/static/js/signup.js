...
usernameInput.onblur = () => {
   ...
   $.ajax({
    type:"get",
      url:`/check/username?username=${username}`, /*/check/username까지가 맵핑주소*/
      dataType:"text",//응답받을때의 데이터 형태 나중에 text가 json이 될 예정
      success:(response)=>{
        if(response == "true"){
         signupFlag[4] = false;
         usernameCheckMsg.innerHTML = `<td colspan="2">${username}은(는) 이미 존재하는 사용자 이름 입니다.</td>`;
      }else{
         signupFlag[4] = true;
         usernameCheckMsg.innerHTML = `<td colspan="2">${username}은(는) 가입 할 수 있는 사용자 이름 입니다.</td>`;
      }
      },
      error:errorMessage
